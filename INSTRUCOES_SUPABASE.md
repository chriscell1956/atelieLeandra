...
## Atualização: Tabela de Depoimentos (Cashback)
Para salvar os feedbacks dos clientes, execute este comando:
...
Se você quer corrigir o erro mas manter o histórico de visitas que já existe, use este comando.
Ele apenas troca a regra de segurança, sem apagar a tabela.

```sql
-- 1. Remove a regra antiga que bloqueia a exclusão
alter table public.access_logs
drop constraint if exists access_logs_product_id_fkey;

-- 2. Cria a regra nova que permite excluir
alter table public.access_logs
...
**Nota:** Se aparecer erro dizendo que a tabela não existe, é porque ela já foi apagada. Nesse caso, use o comando de recriar abaixo.

## Opção 2: Recriar Tabela (Se a de cima falhar)
Só use se a opção acima der erro de "relation does not exist".
...
alter table public.access_logs enable row level security;
create policy "Visitantes criam logs" on public.access_logs for insert with check (true);
create policy "Admin vê logs" on public.access_logs for select using (auth.role() = 'authenticated');
```
```
