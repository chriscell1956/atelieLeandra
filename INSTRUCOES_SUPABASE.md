...
## Tabela Sumiu? (Erro "relation does not exist")
Se você rodou o comando acima e apareceu o erro `relation "public.access_logs" does not exist`, significa que **a tabela não existe mais**. 
Nesse caso, não há dados para salvar (eles já se foram). Para consertar o site e voltar a ter logs, você deve recriar a tabela com este comando:
...
-- Reativar permissões (caso necessário)
do $$
begin
  if not exists (select from pg_tables where schemaname = 'public' and tablename = 'access_logs' and rowsecurity = true) then
    alter table public.access_logs enable row level security;
